...
# experiment result
throughput_file = 'apo042/src/throughput.txt'
...
def retrieve_from_different_nodes(nodes, tries=10, run_test=0):
    ...
    if run_test:
        throughput = (tries*2) / overall_time
        print(f"Throughput for {tries*2} requests: {throughput}")
...
def main(args):
    run_test = 0 # set the value as 1  to conduct experiment for finding throughput
    ...
    if run_test:  # throughput calculation experiment
        for _ in range(10):
            retrieve_from_different_nodes(nodes, 250, run_test)
